...
namespace PaintInEditor
{
	...
	public static class CwEditorDilate
	{
		...
		public static void Dilate(RenderTexture texture, Mesh mesh, int channel, int highQualitySteps)
		{
			...
			var mask      = CwCommon.GetRenderTexture(new RenderTextureDescriptor(texture.width, texture.height, format));
			var deltas    = CwCommon.GetRenderTexture(new RenderTextureDescriptor(texture.width, texture.height, format));
			var swap      = CwCommon.GetRenderTexture(texture);
			...
			dilateMaterial.SetVector(_CwCoord, CwCommon.GetChannelVector(channel));
			...
			CwCommon.ReleaseRenderTexture(mask);
			CwCommon.ReleaseRenderTexture(deltas);
