...
{
	public class CwEditorBrush_Sticker : CwEditorBrush
	{
		...
		{
			return CwPaintInEditor.Settings.Sticker.Size;
		}

		public override float GetAngle()
		{
			return CwPaintInEditor.Settings.Sticker.Angle;
		}

		public override void DrawSettings(CwPaintInEditor.SettingsData settings)
		{
			settings.Sticker.Tint    = EditorGUILayout.ColorField("Tint", settings.Sticker.Tint);
			settings.Sticker.Opacity = EditorGUILayout.Slider("Opacity", settings.Sticker.Opacity, 0.0f, 1.0f);
			settings.Sticker.Size    = EditorGUILayout.Slider("Size", settings.Sticker.Size, 1.0f, 1000.0f);
			settings.Sticker.Angle   = EditorGUILayout.Slider("Angle", settings.Sticker.Angle, -360.0f, 360.0f);

			CwEditor.Separator();

			settings.Sticker.JitterPosition   = EditorGUILayout.Slider("Jitter Position"   , settings.Sticker.JitterPosition  , 0.0f, 1000.0f);
			settings.Sticker.JitterSize       = EditorGUILayout.Slider("Jitter Size"       , settings.Sticker.JitterSize      , 0.0f, 1000.0f);
			settings.Sticker.JitterAngle      = EditorGUILayout.Slider("Jitter Angle"      , settings.Sticker.JitterAngle     , 0.0f,  360.0f);
			settings.Sticker.JitterOpacity    = EditorGUILayout.Slider("Jitter Opacity"    , settings.Sticker.JitterOpacity   , 0.0f,    1.0f);
			settings.Sticker.JitterHue        = EditorGUILayout.Slider("Jitter Hue"        , settings.Sticker.JitterHue       , 0.0f,    1.0f);
			settings.Sticker.JitterSaturation = EditorGUILayout.Slider("Jitter Saturation" , settings.Sticker.JitterSaturation, 0.0f,    1.0f);
			settings.Sticker.JitterLightness  = EditorGUILayout.Slider("Jitter Lightness"  , settings.Sticker.JitterLightness, 0.0f,    1.0f);

			CwEditor.Separator();

			settings.Sticker.WriteAlbedo     = EditorGUILayout.Toggle("Write Albedo"    , settings.Sticker.WriteAlbedo    );
			settings.Sticker.WriteOpacity    = EditorGUILayout.Toggle("Write Opacity"   , settings.Sticker.WriteOpacity   );
			settings.Sticker.WriteNormal     = EditorGUILayout.Toggle("Write Normal"    , settings.Sticker.WriteNormal    );
			settings.Sticker.WriteMetallic   = EditorGUILayout.Toggle("Write Metallic"  , settings.Sticker.WriteMetallic  );
			settings.Sticker.WriteOcclusion  = EditorGUILayout.Toggle("Write Occlusion" , settings.Sticker.WriteOcclusion );
			settings.Sticker.WriteSmoothness = EditorGUILayout.Toggle("Write Smoothness", settings.Sticker.WriteSmoothness);
			settings.Sticker.WriteEmission   = EditorGUILayout.Toggle("Write Emission"  , settings.Sticker.WriteEmission  );
			settings.Sticker.WriteHeight     = EditorGUILayout.Toggle("Write Height"    , settings.Sticker.WriteHeight    );
		}
		...
		public override void DrawBrush(CwPaintInEditor.SettingsData settings, CwEditorPaintable paintable, CwEditorTextureProfile.Subset subset, double oldDistance, Vector2 oldPoint, Vector2 newPoint, bool down)
		{
			...
			var point   = oldPoint + Random.insideUnitCircle * settings.Sticker.JitterPosition;
			var angle   = Random.Range(-1.0f, 1.0f) * settings.Sticker.JitterAngle + settings.Sticker.Angle;
			var opacity = Mathf.Clamp01(Random.Range(-1.0f, 1.0f) * settings.Sticker.JitterOpacity + settings.Sticker.Opacity);
			var deltaH  = Random.Range(-1.0f, 1.0f) * settings.Sticker.JitterHue;
			var deltaS  = Random.Range(-1.0f, 1.0f) * settings.Sticker.JitterSaturation;
			var deltaV  = Random.Range(-1.0f, 1.0f) * settings.Sticker.JitterLightness;
			...
			if (blend == BlendType.Replace)
			{
				...
				{
					decalMaterial.shaderKeywords = new string[] { "CW_REPLACE", "CW_NORMAL", "CW_STICKER" };
				}
				else
				{
					decalMaterial.shaderKeywords = new string[] { "CW_REPLACE", "CW_RGBA", "CW_STICKER" };
				}
			}
			...
			if (blend == BlendType.Combine)
			{
				if (subset.Name == "Normal")
				{
					decalMaterial.shaderKeywords = new string[] { "CW_COMBINE", "CW_NORMAL", "CW_STICKER" };
				}
				else
				{
					decalMaterial.shaderKeywords = new string[] { "CW_COMBINE", "CW_RGBA", "CW_STICKER" };
				}
			}
			...
			if (blend == BlendType.Erase)
			{
				...
				if (subset.Name == "Normal")
				{
					decalMaterial.shaderKeywords = new string[] { "CW_ERASE", "CW_NORMAL", "CW_STICKER" };
				}
				else
				{
					decalMaterial.shaderKeywords = new string[] { "CW_ERASE", "CW_RGBA", "CW_STICKER" };
				}
			}

			decalMaterial.SetTexture(_CwTileTexture, Texture2D.whiteTexture);
			...
			decalMaterial.SetTexture(_CwPaintShape, opacityTexture != null ? opacityTexture : Texture2D.whiteTexture);
			decalMaterial.SetVector(_CwPaintShapeChannel, CwCommon.GetTextureChannel(opacityTexture));
			...
			decalMaterial.SetFloat(_CwTileScale, 1.0f);
			decalMaterial.SetFloat(_CwTileStrength, 0.0f);
			decalMaterial.SetFloat(_CwTileTransition, 1.0f);
			decalMaterial.SetTexture(_CwTileTexture, Texture2D.whiteTexture);
			...
			switch (subset.Name)
			{
				case "Albedo": if (albedoTexture != null && settings.Sticker.WriteAlbedo == true)
				{
					float h, s, v; Color.RGBToHSV(settings.Sticker.Tint, out h, out s, out v);
					...
					decalMaterial.SetTexture(_CwPaintTexture, albedoTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Opacity": if (replaceOpacityTexture != null && settings.Sticker.WriteOpacity == true)
				{
					decalMaterial.SetTexture(_CwPaintTexture, replaceOpacityTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Normal": if (normalTexture != null && settings.Sticker.WriteNormal == true)
				{
					decalMaterial.SetTexture(_CwPaintTexture, normalTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Metallic": if (metallicTexture != null && settings.Sticker.WriteMetallic == true)
				{
					decalMaterial.SetTexture(_CwPaintTexture, metallicTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Occlusion": if (occlusionTexture != null && settings.Sticker.WriteOcclusion == true)
				{
					decalMaterial.SetTexture(_CwPaintTexture, occlusionTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Smoothness": if (smoothnessTexture != null && settings.Sticker.WriteSmoothness == true)
				{
					decalMaterial.SetTexture(_CwPaintTexture, smoothnessTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Emission": if (emissionTexture != null && settings.Sticker.WriteEmission == true)
				{
					decalMaterial.SetTexture(_CwPaintTexture, emissionTexture);
					CwPaintInEditor.PaintNow(decalMaterial);
				}
				break;

				case "Height": if (heightTexture != null && settings.Sticker.WriteHeight == true)
				{
...
namespace PaintInEditor
{
	using CW.Common;
	using UnityEditor;
	using TARGET = CwEditorBrush_Sticker;

	[CanEditMultipleObjects]
	[CustomEditor(typeof(TARGET))]
	public class CwEditorBrush_Sticker_Editor : CwEditorBrush_Editor
	{
		...
		{
			var tgt  = (CwEditorBrush_Sticker)target;
		...
		[MenuItem("Assets/Create/CW/Paint in Editor/Brush (Sticker)")]
		private static void CreateAsset_Paint()
		{
			var asset = CreateAsset_AndBuildTempTextures<CwEditorBrush_Sticker>(asset =>
				{
		...
		private void RegenerateThumb(CwEditorBrush_Sticker tgt, int size)
		{
			thumb = CwEditorThumb.Generate(size, CwCommon.GetCubeMesh(), new Vector2(1.0f, 1.0f),
